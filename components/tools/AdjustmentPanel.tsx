...
const AdjustmentPanel: React.FC = () => {
    ...
    const handleFilterChange = (filter: keyof typeof localFilters, value: number) => {
        ...
        // Apply filter directly to the image element for preview
        if (imgRef.current) {
            imgRef.current.style.filter = buildFilterString({ ...localFilters, [filter]: value });
        }
    };
    ...
    const handleApply = () => {
        handleApplyLocalAdjustments();
        if (imgRef.current) {
            imgRef.current.style.filter = 'none'; // Reset preview filter
        }
    }
